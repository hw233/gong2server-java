...
@SdkGroup({ SdkConst.CC_SDK_IOS_HD, SdkConst.CC_SDK_ANDROID_HD, SdkConst.CC_SDK_ANDROID_GOOGLE })
@Component("sdkHdService")
public class SdkHdService extends SdkBaseService implements SdkService {
	...
	public Object payCallback(Map<String, Object> requestParams) {
		LoggerUtils.info("SdkHDService payCallback", requestParams);
		// 支付宝和微信都有这个字段
		String out_trade_no = FsGameLoginUtils.getMapString(requestParams, "out_trade_no", "");
		if (StringUtils.isEmpty(out_trade_no)) {
			// 微信直接可能获取不到参数
			LoggerUtils.info("SdkHDService payCallback out_trade_no null");
